...
void CameraControlComponent::handle_dispatch() {
  ...
  if (blackboard->sim_state_blackboard->camera_type == types::eCameraType::FREE) {
    Magnum::Matrix4 yaw_mat = Magnum::Matrix4::rotation(Magnum::Deg(blackboard->keyboard_input_blackboard->commanded_yaw), Magnum::Vector3::yAxis());
    ...
    active_camera._revolut->rotateLocal(Magnum::Deg(blackboard->keyboard_input_blackboard->commanded_yaw), Magnum::Vector3::yAxis());
  }

  active_camera._revolut->rotateLocal(Magnum::Deg(blackboard->keyboard_input_blackboard->commanded_pitch), Magnum::Vector3::xAxis());
  active_camera._revolut->rotateLocal(Magnum::Deg(blackboard->keyboard_input_blackboard->commanded_roll),  Magnum::Vector3::zAxis());
  ...
  active_camera._mount->translate(
    right * blackboard->keyboard_input_blackboard->commanded_translation.x() +
    up    * blackboard->keyboard_input_blackboard->commanded_translation.y() +
    back  * blackboard->keyboard_input_blackboard->commanded_translation.z()
  );
}
